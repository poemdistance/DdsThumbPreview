...
extern const wchar_t* g_currentLogDir;
...
inline void printlog(const wchar_t* format, ...) {
    ...
    try {
        // 如果 C# 没有传日志目录，或者处于非调试状态，直接退回系统标准输出，不写文件
        if (!g_currentLogDir) {
            va_list args;
        ...
        // 1. 拼接完整的日志路径
        std::wstring fullLogPath = std::wstring(g_currentLogDir);
        if (!fullLogPath.empty() && fullLogPath.back() != L'\\' && fullLogPath.back() != L'/') {
            fullLogPath += L"\\";
        }
        fullLogPath += L"preview_callback.log";

        // 2. 以追加模式打开文件
        std::wofstream logFile(fullLogPath, std::ios::app);
        if (logFile.is_open()) {
            
            // 3. 处理变长参数列表（完美支持 %zu, %ls 等所有 myOwnWPrintf 语法）
            wchar_t buffer[2048]; // 放大缓冲区以防单条大日志溢出
            va_list args;
            va_start(args, format);
            ...
            // 4. 判断是否需要打印时间戳
            // 使用一个静态变量记录“上一行是不是刚刚换过行”
            static bool needsTimestamp = true;

            std::wstring logOutput;
            std::wstring inputStr(buffer);
            
            // 扫描解析输入的文本，确保即使 texconv 内部片段拼接，时间戳也不会错位
            size_t startPos = 0;
            while (startPos < inputStr.length()) {
                if (needsTimestamp) {
                    ...
                    logFile << L"[" << std::setw(2) << std::setfill(L'0') << timeinfo.tm_hour << L":"
                            ...
                            << std::setw(3) << std::setfill(L'0') << ms.count() << L"] [texconv] ";
                    needsTimestamp = false;
                }

                // 寻找内部换行符
                size_t nextLineBreak = inputStr.find(L'\n', startPos);
                if (nextLineBreak != std::wstring::npos) {
                    // 如果找到了换行符，把换行符之前的内容写进去，并打上换行
                    logFile << inputStr.substr(startPos, nextLineBreak - startPos + 1);
                    needsTimestamp = true; // 下一次循环或下一次调用时需要打时间戳
                    startPos = nextLineBreak + 1;
                } else {
                    // 如果后面没有换行符了，直接把剩下的当做片段写进去
                    logFile << inputStr.substr(startPos);
                    break;
                }
            }
            
            logFile.flush(); // 实时刷新，防止程序崩溃时日志丢失
        }
    }
    catch (...) {}
}
...
namespace
{
    ...
    {
        printlog(L" (%zux%zu", info.width, info.height);

        if (TEX_DIMENSION_TEXTURE3D == info.dimension)
            printlog(L"x%zu", info.depth);

        if (info.mipLevels > 1)
            printlog(L",%zu", info.mipLevels);

        if (info.arraySize > 1)
            printlog(L",%zu", info.arraySize);
        ...
        case TEX_DIMENSION_TEXTURE1D:
            printlog(L"%ls", (info.arraySize > 1) ? L" 1DArray" : L" 1D");
            break;

        case TEX_DIMENSION_TEXTURE2D:
            if (info.IsCubemap())
            {
                printlog(L"%ls", (info.arraySize > 6) ? L" CubeArray" : L" Cube");
            }
            else
            {
                printlog(L"%ls", (info.arraySize > 1) ? L" 2DArray" : L" 2D");
            }
            break;
    ...
    void PrintUsage(bool full = false) noexcept
    {
        ...
        printlog(L"%ls", s_usage);

        if (!full)
            return;

        printlog(L"%ls", s_fullUsage);
        ...
        if (GetDXGIFactory(dxgiFactory.GetAddressOf()))
        {
                ...
                ++adapterIndex)
            {
                ...
                {
                    printlog(L"      %u: VID:%04X, PID:%04X - %ls\n", adapterIndex, desc.VendorId, desc.DeviceId, desc.Description);
                }
            }
        }
    }
...
// texconv.cpp原始的main函数
int __cdecl OriginMain(_In_ int argc, _In_z_count_(argc) wchar_t* argv[],
    const uint8_t* ddsBuffer, // 默认为空，不影响原有的普通调用
    size_t ddsSize,
    uint8_t** outPngBuffer,      // [输出] 用于返回 PNG 内存指针
    size_t* outPngSize // [输出] 用于返回 PNG 内存大小 
    )
{


    bool loadFromMemory = false;
    if (ddsBuffer && ddsSize > 0) {
        loadFromMemory = true;
    }
    ...
    if (FAILED(hr))
    {
        // 如果错误码是 0x80010106，说明 C# 已经初始化过了，我们直接原谅它，继续往下走！
        if (hr == static_cast<HRESULT>(0x80010106))
        {
            printlog(L"COM is already initialized by host (STA/MTA clash ignored).\n");
        }
        else
        {
            printlog(L"Failed to initialize COM (%08X%ls)\n", static_cast<unsigned int>(hr), GetErrorDesc(hr));
            return 1; // 真正的其他 COM 错误才拦截退出
        }
    ...
    for (int iArg = 1; iArg < argc; ++iArg)
    {
        ...
        if (allowOpts && (('-' == pArg[0]) || ('/' == pArg[0])))
        {
            ...
            else
            {
                ...
                if (!dwOption)
                {
                    ...
                    {
                        printlog(L"ERROR: did you mean `--%ls` (with two dashes)?\n", pArg);
                        return 1;
                    }
                }
            }
            ...
            {
            case 0:
                printlog(L"ERROR: Unknown option: `%ls`\n\nUse %ls --help\n", pArg, g_ToolName);
                return 1;
            ...
            default:
                ...
                {
                    printlog(L"ERROR: Duplicate option: `%ls`\n\n", pArg);
                    return 1;
                }
            ...
            case OPT_WIDTH:
                ...
                {
                    printlog(L"Invalid value specified with -w (%ls)\n\n", pValue);
                    PrintUsage();
                    return 1;
                }
                break;
            ...
            case OPT_HEIGHT:
                ...
                {
                    printlog(L"Invalid value specified with -h (%ls)\n", pValue);
                    printf("\n");
                    PrintUsage();
                    return 1;
                }
            ...
            case OPT_MIPLEVELS:
                ...
                {
                    printlog(L"Invalid value specified with -m (%ls)\n\n", pValue);
                    PrintUsage();
                    return 1;
                }
                break;
            ...
            case OPT_FORMAT:
                ...
                if (!format)
                {
                    ...
                    if (!format)
                    {
                        ...
                        default:
                            printlog(L"Invalid value specified with -f (%ls)\n\n", pValue);
                            PrintUsage();
                            return 1;
                        }
                    }
            ...
            case OPT_FILTER:
                ...
                if (!dwFilter)
                {
                    printlog(L"Invalid value specified with -if (%ls)\n\n", pValue);
                    PrintUsage();
                    return 1;
                }
                break;
            ...
            case OPT_ROTATE_COLOR:
                ...
                if (!dwRotateColor)
                {
                    printlog(L"Invalid value specified with -rotatecolor (%ls)\n\n", pValue);
                    PrintUsage();
                    return 1;
                }
                break;
            ...
            case OPT_FILETYPE:
                ...
                if (!FileType)
                {
                    printlog(L"Invalid value specified with -ft (%ls)\n\n", pValue);
                    PrintUsage();
                    return 1;
                }
                break;
            ...
            case OPT_PREMUL_ALPHA:
                ...
                {
                    printlog(L"Can't use -pmalpha and -alpha at same time\n\n");
                    PrintUsage();
                    return 1;
                }
                break;
            ...
            case OPT_TA_WRAP:
                ...
                {
                    printlog(L"Can't use -wrap and -mirror at same time\n\n");
                    PrintUsage();
                    return 1;
                }
            ...
            case OPT_NORMAL_MAP:
            {
                ...
                {
                    printlog(L"Invalid value specified for -nmap (%ls), missing l, r, g, b, or a\n\n", pValue);
                    return 1;
                }
            ...
            case OPT_NORMAL_MAP_AMPLITUDE:
                if (!dwNormalMap)
                {
                    printlog(L"-nmapamp requires -nmap\n\n");
                    PrintUsage();
                    return 1;
                }
                else if (swscanf_s(pValue, L"%f", &nmapAmplitude) != 1)
                {
                    printlog(L"Invalid value specified with -nmapamp (%ls)\n\n", pValue);
                    PrintUsage();
                    return 1;
                }
                else if (nmapAmplitude < 0.f)
                {
                    printlog(L"Normal map amplitude must be positive (%ls)\n\n", pValue);
                    return 1;
                }
                break;
            ...
            case OPT_GPU:
                ...
                {
                    printlog(L"Invalid value specified with -gpu (%ls)\n\n", pValue);
                    PrintUsage();
                    return 1;
                }
                else if (adapter < 0)
                {
                    printlog(L"Invalid adapter index (%ls)\n\n", pValue);
                    PrintUsage();
                    return 1;
                }
                break;
            ...
            case OPT_FEATURE_LEVEL:
                ...
                if (!maxSize)
                {
                    printlog(L"Invalid value specified with -fl (%ls)\n\n", pValue);
                    PrintUsage();
                    return 1;
                }
                break;
            ...
            case OPT_ALPHA_THRESHOLD:
                ...
                {
                    printlog(L"Invalid value specified with -at (%ls)\n\n", pValue);
                    PrintUsage();
                    return 1;
                }
                else if (alphaThreshold < 0.f)
                {
                    printlog(L"-at (%ls) parameter must be positive\n\n", pValue);
                    return 1;
                }
                break;
            ...
            case OPT_ALPHA_WEIGHT:
                ...
                {
                    printlog(L"Invalid value specified with -aw (%ls)\n\n", pValue);
                    PrintUsage();
                    return 1;
                }
                else if (alphaWeight < 0.f)
                {
                    printlog(L"-aw (%ls) parameter must be positive\n\n", pValue);
                    return 1;
                }
                break;
            ...
            case OPT_BC_COMPRESS:
            {
                ...
                {
                    printlog(L"Can't use -bc x (max) and -bc q (quick) at same time\n\n");
                    PrintUsage();
                    return 1;
                }
                ...
                if (!found)
                {
                    printlog(L"Invalid value specified for -bc (%ls), missing d, u, q, or x\n\n", pValue);
                    return 1;
                }
            }
            break;
            ...
            case OPT_WIC_QUALITY:
                if (swscanf_s(pValue, L"%f", &wicQuality) != 1
                    ...
                    || (wicQuality > 1.f))
                {
                    printlog(L"Invalid value specified with -wicq (%ls)\n", pValue);
                    printf("\n");
                    PrintUsage();
                    return 1;
                }
            ...
            case OPT_USE_DX10:
                ...
                {
                    printlog(L"Can't use -dx9 and -dx10 at same time\n\n");
                    PrintUsage();
                    return 1;
                }
                break;
            ...
            case OPT_RECURSIVE:
                if (*pValue)
                {
                    ...
                    {
                        printlog(L"For recursive use -r, -r:flatten, or -r:keep\n\n");
                        PrintUsage();
                        return 1;
                    }
                }
            ...
            case OPT_FILELIST:
            {
                ...
                if (!inFile)
                {
                    printlog(L"Error opening -flist file %ls\n", pValue);
                    return 1;
                }
            ...
            case OPT_PAPER_WHITE_NITS:
                ...
                {
                    printlog(L"Invalid value specified with -nits (%ls)\n\n", pValue);
                    PrintUsage();
                    return 1;
                }
                else if (paperWhiteNits > 10000.f || paperWhiteNits <= 0.f)
                {
                    printlog(L"-nits (%ls) parameter must be between 0 and 10000\n\n", pValue);
                    return 1;
                }
                break;
            ...
            case OPT_PRESERVE_ALPHA_COVERAGE:
                ...
                {
                    printlog(L"Invalid value specified with -keepcoverage (%ls)\n\n", pValue);
                    PrintUsage();
                    return 1;
                }
                else if (preserveAlphaCoverageRef < 0.0f || preserveAlphaCoverageRef > 1.0f)
                {
                    printlog(L"-keepcoverage (%ls) parameter must be between 0.0 and 1.0\n\n", pValue);
                    return 1;
                }
                break;
            ...
            case OPT_SWIZZLE:
                ...
                {
                    printlog(L"Invalid value specified with -swizzle (%ls)\n\n", pValue);
                    PrintUsage();
                    return 1;
                }
                else if (!ParseSwizzleMask(pValue, swizzleElements, zeroElements, oneElements))
                {
                    printlog(L"-swizzle requires a 1 to 4 character mask composed of these letters: r, g, b, a, x, y, w, z, 0, 1\n");
                    return 1;
                }
                break;
        ...
        else if (wcspbrk(pArg, L"?*") != nullptr)
        {
            ...
            {
                printlog(L"No matching files found for %ls\n", pArg);
                return 1;
            }
        }
        else
    ...
    for (auto pConv = conversion.begin(); pConv != conversion.end(); ++pConv)
    {
        ...
        // --- Load source image -------------------------------------------------------
        printlog(L"reading %ls", pConv->szSrc.c_str());
        fflush(stdout);
        ...
        if (_wcsicmp(ext.c_str(), L".dds") == 0 || _wcsicmp(ext.c_str(), L".ddx") == 0)
        {
...
#endif // USE_XBOX_EXTS
            {
                ...
                if (loadFromMemory) {
                    hr = LoadFromDDSMemory(ddsBuffer, ddsSize, ddsFlags, &info, *image);
                    printlog(L"load dds from memory succ");
                }
                else {
            ...
            if (IsTypeless(info.format))
            {
                ...
                if (IsTypeless(info.format))
                {
                    printlog(L" FAILED due to Typeless format %d\n", info.format);
                    retVal = 1;
                    continue;
                }
        ...
        else
        {
            ...
            if (FAILED(hr))
            {
                ...
                if (hr == static_cast<HRESULT>(0xc00d5212) /* MF_E_TOPO_CODEC_NOT_FOUND */)
                {
                    ...
                    {
                        printlog(L"INFO: This format requires installing the HEIF Image Extensions - https://aka.ms/heif\n");
                    }
                    else if (_wcsicmp(ext.c_str(), L".webp") == 0)
                    {
                        printlog(L"INFO: This format requires installing the WEBP Image Extensions - https://apps.microsoft.com/detail/9PG2DK419DRG\n");
                    }
                }
                continue;
            }
        ...
        if (IsPlanar(info.format))
        {
            ...
            if (FAILED(hr))
            {
                printlog(L" FAILED [converttosingleplane] (%08X%ls)\n",
                    static_cast<unsigned int>(hr), GetErrorDesc(hr));
                retVal = 1;
                continue;
            }
        ...
        if (IsCompressed(info.format))
        {
            ...
            if ((info.width % 4) != 0 || (info.height % 4) != 0)
            {
                if (dwOptions & (UINT64_C(1) << OPT_BCNONMULT4FIX))
                {
                    ...
                    if (FAILED(hr))
                    {
                        printlog(L" FAILED [BC non-multiple-of-4 fixup] (%08X%ls)\n",
                            static_cast<unsigned int>(hr), GetErrorDesc(hr));
                        return 1;
                    }
            ...
            if (FAILED(hr))
            {
                printlog(L" FAILED [decompress] (%08X%ls)\n", static_cast<unsigned int>(hr), GetErrorDesc(hr));
                retVal = 1;
                continue;
            }
            ...
            && info.format != DXGI_FORMAT_A8_UNORM)
        {
            ...
            else
            {
                ...
                if (FAILED(hr))
                {
                    printlog(L" FAILED [demultiply alpha] (%08X%ls)\n",
                        static_cast<unsigned int>(hr), GetErrorDesc(hr));
                    retVal = 1;
                    continue;
                }
        ...
        if (dwOptions & ((UINT64_C(1) << OPT_HFLIP) | (UINT64_C(1) << OPT_VFLIP)))
        {
            ...
            if (FAILED(hr))
            {
                printlog(L" FAILED [fliprotate] (%08X%ls)\n", static_cast<unsigned int>(hr), GetErrorDesc(hr));
                return 1;
            }
        ...
        if (info.width != twidth || info.height != theight)
        {
            ...
            if (FAILED(hr))
            {
                printlog(L" FAILED [resize] (%08X%ls)\n", static_cast<unsigned int>(hr), GetErrorDesc(hr));
                return 1;
            }
            ...
            || oneElements[0] != 0 || oneElements[1] != 0 || oneElements[2] != 0 || oneElements[3] != 0)
        {
            ...
            if (FAILED(hr))
            {
                printlog(L" FAILED [swizzle] (%08X%ls)\n", static_cast<unsigned int>(hr), GetErrorDesc(hr));
                return 1;
            }
        ...
        if (dwRotateColor)
        {
            if (dwRotateColor == ROTATE_HDR10_TO_709 || dwRotateColor == ROTATE_P3D65_TO_709)
            {
                ...
                if (FAILED(hr))
                {
                    printlog(L" FAILED [convert] (%08X%ls)\n", static_cast<unsigned int>(hr), GetErrorDesc(hr));
                    return 1;
                }
            ...
            if (FAILED(hr))
            {
                printlog(L" FAILED [rotate color apply] (%08X%ls)\n",
                    static_cast<unsigned int>(hr), GetErrorDesc(hr));
                return 1;
            }
        ...
        if (dwOptions & UINT64_C(1) << OPT_TONEMAP)
        {
            ...
            if (FAILED(hr))
            {
                printlog(L" FAILED [tonemap maxlum] (%08X%ls)\n", static_cast<unsigned int>(hr), GetErrorDesc(hr));
                return 1;
            }
            ...
            if (FAILED(hr))
            {
                printlog(L" FAILED [tonemap apply] (%08X%ls)\n", static_cast<unsigned int>(hr), GetErrorDesc(hr));
                return 1;
            }
        ...
        if (dwOptions & (UINT64_C(1) << OPT_NORMAL_MAP))
        {
            ...
            if (FAILED(hr))
            {
                printlog(L" FAILED [normalmap] (%08X%ls)\n", static_cast<unsigned int>(hr), GetErrorDesc(hr));
                return 1;
            }
            ...
            && HasAlpha(info.format))
        {
            ...
            if (FAILED(hr))
            {
                printlog(L" FAILED [colorkey] (%08X%ls)\n", static_cast<unsigned int>(hr), GetErrorDesc(hr));
                return 1;
            }
        ...
        if (dwOptions & (UINT64_C(1) << OPT_INVERT_Y))
        {
            ...
            if (FAILED(hr))
            {
                printlog(L" FAILED [inverty] (%08X%ls)\n", static_cast<unsigned int>(hr), GetErrorDesc(hr));
                return 1;
            }
        ...
        if (dwOptions & (UINT64_C(1) << OPT_RECONSTRUCT_Z))
        {
            ...
            if (FAILED(hr))
            {
                printlog(L" FAILED [reconstructz] (%08X%ls)\n", static_cast<unsigned int>(hr), GetErrorDesc(hr));
                return 1;
            }
        ...
        if ((!tMips || info.mipLevels != tMips || preserveAlphaCoverage) && (info.mipLevels != 1))
        {
            ...
            if (FAILED(hr))
            {
                printlog(L" FAILED [copy to single level] (%08X%ls)\n",
                    static_cast<unsigned int>(hr), GetErrorDesc(hr));
                return 1;
            }
            ...
            if (cimage && (tMips == 1))
            {
                ...
                if (FAILED(hr))
                {
                    printlog(L" FAILED [copy compressed to single level] (%08X%ls)\n", static_cast<unsigned int>(hr), GetErrorDesc(hr));
                    return 1;
                }
        ...
        if ((!tMips || info.mipLevels != tMips) && (info.width > 1 || info.height > 1 || info.depth > 1))
        {
            ...
            if (FAILED(hr))
            {
                printlog(L" FAILED [mipmaps] (%08X%ls)\n", static_cast<unsigned int>(hr), GetErrorDesc(hr));
                return 1;
            }
        ...
        if (preserveAlphaCoverage && info.mipLevels != 1 && (info.dimension != TEX_DIMENSION_TEXTURE3D))
        {
            ...
            if (FAILED(hr))
            {
                printlog(L" FAILED [keepcoverage] (%08X%ls)\n", static_cast<unsigned int>(hr), GetErrorDesc(hr));
                return 1;
            }
            ...
            && info.format != DXGI_FORMAT_A8_UNORM)
        {
            ...
            else
            {
                ...
                if (FAILED(hr))
                {
                    printlog(L" FAILED [premultiply alpha] (%08X%ls)\n", static_cast<unsigned int>(hr), GetErrorDesc(hr));
                    retVal = 1;
                    continue;
                }
        ...
        if (FileType == CODEC_DDS)
        {
            if (dxt5nm || dxt5rxgb)
            {
                ...
                if (dxt5nm)
                {
                    ...
                    if (FAILED(hr))
                    {
                        printlog(L" FAILED [DXT5nm] (%08X%ls)\n", static_cast<unsigned int>(hr), GetErrorDesc(hr));
                        return 1;
                    }
                }
                else
                {
                    ...
                    if (FAILED(hr))
                    {
                        printlog(L" FAILED [DXT5 RXGB] (%08X%ls)\n", static_cast<unsigned int>(hr), GetErrorDesc(hr));
                        return 1;
                    }
                }
            ...
            if (IsCompressed(tformat))
            {
                ...
                else
                {
                    ...
                    if (FAILED(hr))
                    {
                        printlog(L" FAILED [compress] (%08X%ls)\n", static_cast<unsigned int>(hr), GetErrorDesc(hr));
                        retVal = 1;
                        continue;
                    }
        ...
        // --- Save result -------------------------------------------------------------
        {
            ...
            {
                // 强制将格式视为 WIC PNG
                const WICCodecs codec = WIC_CODEC_PNG;
                const size_t nimages = (dwOptions & (UINT64_C(1) << OPT_WIC_MULTIFRAME)) ? nimg : 1;

                // 准备 DirectXTex 的二进制数据容器
                DirectX::Blob blob;

                printlog(L"writing to output memory buffer (PNG format)...");
                fflush(stdout);

                // 调用内存保存 API，代替 SaveToWICFile
                hr = SaveToWICMemory(img, nimages, WIC_FLAGS_NONE, GetWICCodec(codec), blob, nullptr,
                    [&](IPropertyBag2* props)
                    {
                        if (wicQuality >= 0.f)
                        {
                ...
                {
                    // 将 Blob 内的数据长度赋给外层
                    *outPngSize = blob.GetBufferSize();

                    // 使用 CoTaskMemAlloc 分配内存，确保跨 DLL 边界时 C# 可以用 Marshal.FreeCoTaskMem 安全释放
                    *outPngBuffer = static_cast<uint8_t*>(::CoTaskMemAlloc(blob.GetBufferSize()));

                    if (*outPngBuffer)
                    {
                        // 把数据拷贝到新分配的内存空间中
                        memcpy(*outPngBuffer, blob.GetBufferPointer(), blob.GetBufferSize());

                        // 测试逻辑, 同时将这个内存中的 PNG 落盘到日志目录，方便肉眼排查
                        //if (g_currentLogDir)
                        //{
                        //    try
                        //    {
                        //        std::wstring testPngPath = std::wstring(g_currentLogDir);
                        //        if (!testPngPath.empty() && testPngPath.back() != L'\\' && testPngPath.back() != L'/') {
                        //            testPngPath += L"\\";
                        //        }
                        //        testPngPath += L"cpp_debug_test.png"; // 测试文件名

                        //        // 以二进制覆盖模式写入文件
                        //        std::ofstream testFile(testPngPath, std::ios::binary | std::ios::trunc);
                        //        if (testFile.is_open())
                        //        {
                        //            testFile.write(reinterpret_cast<const char*>(blob.GetBufferPointer()), blob.GetBufferSize());
                        //            testFile.close();
                        //            printlog(L" [Debug] Successfully dumped PNG to %ls\n", testPngPath.c_str());
                        //        }
                        //        else
                        //        {
                        //            printlog(L" [Debug] Failed to open dump file at %ls\n", testPngPath.c_str());
                        //        }
                        //    }
                        //    catch (...)
                        //    {
                        //        printlog(L" [Debug] Exception occurred while dumping PNG file.\n");
                        //    }
                        //}
                    }
                    else
            ...
            if (keepRecursiveDirs && !pConv->szFolder.empty())
            {
                ...
                if (ec)
                {
                    printlog(L" get full path FAILED (%hs)\n", ec.message().c_str());
                    retVal = 1;
                    continue;
                }
                ...
                {
                    printlog(L" directory creation FAILED (%08X%ls)\n",
            ...
            // Write texture
            printlog(L"writing %ls", destName.c_str());
            fflush(stdout);
